...
SELECT COUNT(r.rental_id) as nr_rental, CONCAT(c.first_name, ' ', c.last_name) AS Name
FROM rental r
JOIN customer c
USING (customer_id)
...
SELECT film_id, COUNT(actor_id) as number_of__actors
FROM film_actor
GROUP BY film_id
...
SELECT c.name as genre_name,  fc.film_id, f.title
